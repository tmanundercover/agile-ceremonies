...
import { StandupModalProps, StandupData } from './types';
import { Teammate } from '../models';
import ActualStandupModal from './ActualStandupModal';
...
const StandupModal: React.FC = () => {
  const [isModalVisible, setIsModalVisible] = useState<boolean>(false);
  const [submittedData, setSubmittedData] = useState<StandupData | null>(null);
  
  const demoTeammate: Teammate = {
    id: '1',
    name: 'Demo User',
  ...
  const handleCloseModal = (data: StandupData | { name?: string }): void => {
    setIsModalVisible(false);
    if ('name' in data && data.name) { // Only set if we have actual data (not a cancel)
      setSubmittedData(data as StandupData);
    }
  };
...
import { Teammate, StandupData } from './types';
...
const MyComponent = () => {
  ...
  const handleStandupSubmit = (standupData: StandupData | { name?: string }) => {
    setShowModal(false);
